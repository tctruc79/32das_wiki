...
SRC="/Volumes/DATA/.CloudStorage/Data/OneDrive2-Personal/MAE/STUDY/MODULE2/9. [32_DAS] - DATA SCIENCE AND APPLICATIONS/wiki"
DEST="$(cd "$(dirname "$0")" && pwd)/content"

rm -rf "$DEST"
mkdir -p "$DEST"

for d in sources concepts people synthesis; do
  if [ -d "$SRC/$d" ]; then
    cp -R "$SRC/$d" "$DEST/$d"
  fi
done
